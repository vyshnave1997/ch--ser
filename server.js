...
io.on('connection', async (socket) => {
  ...
  // Send existing messages from database to the newly connected user
  ...
  socket.on('setUsername', (username) => {
    ...
    saveMessageToDatabase(joinMessage); // Save system message to database
  });

  socket.on('chatMessage', (data) => {
    io.emit('chatMessage', data);
    saveMessageToDatabase(data); // Save user message to database
  });
  ...
  socket.on('disconnect', () => {
    ...
    saveMessageToDatabase(leaveMessage); // Save system message to database
  });
});
